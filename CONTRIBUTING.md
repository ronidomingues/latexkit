...
```bash
node bin/latexkit.js new article /tmp/prova --yes \
  --title "Teste" --author "Autor" --institution "UFX" --city "Cidade"
cd /tmp/prova && node /caminho/para/latexkit/bin/latexkit.js build --verbose
```

## O que voce precisa instalado

O minimo e Node 20.10 e uma instalacao de LaTeX com `abntex2`. Rode
`node bin/latexkit.js doctor` para ver o que falta na sua maquina e como
instalar.
...
1. **Nenhum `.tex` de template contem placeholder `{{...}}`.** Chaves sao
   sintaxe de LaTeX. Todo dado variavel chega pelas macros `\lk*` geradas em
...
- Sem dependencias de runtime. E o que mantem o `npx latexkit` instantaneo;
